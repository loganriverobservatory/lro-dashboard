...
const updateLineCoordinates = async () => {
  ...
  // On narrow screens the diagram sits at its min-width and overflows the
  // viewport, so the wrapper scrolls horizontally. The fixed horizontal offsets
  // used for the orange Cutler inflows then land right at the card's edge,
  // leaving a stub of horizontal line poking out beyond the card. When that
  // happens, drop the orange lines straight down from the card bottom instead,
  // with no horizontal segment. Desktop routing is unchanged.
  const wrapper = gridContainerRef.value
  const isNarrow = wrapper.scrollWidth > wrapper.clientWidth + 1
  const cardBottomOffset = 32 // half a node card's height; matches existing usage

  const lbElement = getMarkerCenter('little_bear')
  if (isNarrow) {
    const dropX = lbElement.x - 12
    paths.value.cutlerLittleBear =
      `M ${dropX},${lbElement.y + cardBottomOffset} ` + `L ${dropX},${reservoirTopY}`
  } else {
  ...
  if (isNarrow) {
    const dropX = scmElement.x + 12
    paths.value.cutlerSpringCreek =
      `M ${dropX},${scmElement.y + cardBottomOffset} ` + `L ${dropX},${reservoirTopY}`
  } else {
