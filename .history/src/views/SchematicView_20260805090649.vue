...
const COL_WIDTH = 560
// Was 170 - too tight for a 2-line wrapped station name at SchematicNode.vue's 2rem card-name
// size, which made neighboring rows visually run into each other ("smashed together" cards).
// Paired with that file's new 2-line clamp (which bounds worst-case card height), this gives
// enough clearance for the common 2-line-name case without needing per-card variable spacing.
...
const vfEdges = computed<VFEdge[]>(() => {
  ...
  const rowOf = (id: string) => layout.get(id)?.row ?? 0
  ...
  //
  // Within the vertical case, which end is physically on top isn't fixed - the main channel
  // always flows source-above/target-below (file order = downstream order), but a branch
  // CHAIN link (e.g. lower-logan's bsf_before_conf, attached below its own anchor bsf_ext)
  // is a tributary/link edge that needs to point back UP into an anchor sitting above it.
  // Comparing row, not just assuming source-above, and choosing s-top/t-bottom for that
  // below-to-above case is what keeps that connector a short straight line instead of
  // routing the long way around the cards (the overlapping-arrow bug).
  function makeEdge(
    id: string,
    ...
    if (sourceCol < targetCol) {
      ...
      edgeShape = 'smoothstep'
    } else if (rowOf(source.id) > rowOf(target.id)) {
      // Same column, but source sits BELOW target (a chain link pointing back up into its
      // anchor) - flip to the top-facing pair instead of the downstream-flow default.
      sourceHandle = 's-top'
