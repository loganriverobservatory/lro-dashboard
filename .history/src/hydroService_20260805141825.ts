...
export interface SchematicNode {
  ...
  returnsTo?: string
  // mainstem/tributary only: set true on a node that RECAPS a station whose true home is a
  // different schematic page (e.g. lower-logan.json's recap of Upper Logan Canyon's Guinavah/
  // Dewitt/Water Lab stations at its own top, or its "bsf_before_conf"/"little_bear" preview
  // cards for Blacksmith Fork River / Little Bear River). Excludes this node from
  // buildSchematicEntries() below, so the station is attributed to exactly one system (its
  // real home page) instead of whichever page happens to appear first in manifest.json. Without
  // this, a station duplicated across pages could be silently dropped from the Map/List system
  // filter's real home page - the exact bug this field was added to fix.
...
function buildSchematicEntries(pages: SchematicPages | null | undefined): SchematicOrderEntry[] {
  ...
  pages.slugs.forEach((slug, pageIndex) => {
    ...
    page.nodes.forEach((node) => {
      if (node.kind === 'junction' || node.kind === 'link' || node.excludeFromOrder) return
