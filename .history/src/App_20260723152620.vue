...
// navigating to a schematic page (the header's Schematic icon, the schematic page's own tab
// bar, a node's "view full system" button, a direct URL, browser back/forward) to remember to
// update it individually. Without this, currentView could get stuck on whatever view was active
// before - keeping List/Map's header item highlighted and the FILTER STATIONS section visible
// even after navigating away to a schematic page.
...
async function loadConfig() {
  ...
  if (config) {
    ...
    dashboardTitle.value =
      typeof config.dashboardTitle === 'string' ? config.dashboardTitle : undefined
