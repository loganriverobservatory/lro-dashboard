...
These are plain static files served directly by the web app — edit one, commit it, and the next time someone loads that schematic page they'll see your change. There's no build step, no publishing pipeline, and no live-data fetch involved (that's different from `config/station_config.yaml`, which drives station identity/colors and _is_ published through a GitHub Action — this folder is unrelated to that pipeline).
...
Each node is one card on the diagram (except `junction`, which draws as a small dot). List them in the order the water actually flows — that order _is_ the layout.
...
| `linkTo`           | no                                                            | A schematic page slug (any slug listed in `manifest.json`) to navigate to on click. On a normal node this adds a "view full system" button below its card; on a `link` node this _is_ the card.                                                                                                                                                                                                                                                                                                                                                         |
...
| `excludeFromOrder` | `mainstem`/`tributary` only                                   | Set `true` when this node is a duplicate preview/recap of a station whose real home is a _different_ schematic page (see "Cross-page duplicates" below). Without this, the Map/List view's system filter attributes a duplicated station to whichever page happens to come first in `manifest.json`, which can silently hide it from its actual system's filter.                                                                                                                                                                                        |
...
A few stations are intentionally drawn on more than one page — e.g. `lower-logan.json` recaps Guinavah Campground and Water Lab from the bottom of `upper-logan.json` so it reads as a complete standalone page, and it shows a preview card of Blacksmith Fork River's and Little Bear River's own final stations so a reader doesn't have to jump pages to see the confluence.

If a station is duplicated like this, exactly one of its copies must be the "real" one and every other copy must set `"excludeFromOrder": true`. This isn't just cosmetic: the Map/List view's system filter builds its station-to-page mapping by walking every page in `manifest.json`'s order and matching by name, so an un-flagged duplicate gets silently claimed by whichever page happens to come first — which can make that station disappear entirely from its actual system's filter. This exact bug happened twice in practice (Blacksmith Fork's and Little Bear's final stations, and separately Cutler Reservoir, which is drawn on both `lower-logan.json` and `little-bear.json`).

When adding a new cross-page duplicate: pick whichever page the station's live identity most belongs to as the "real" copy, and set `excludeFromOrder: true` on every other copy.
...
```json
{
  ...
  "nodes": [
    {
      "id": "hollow_rd",
      "name": "Blacksmith Fork River: Hollow Road",
      "stationId": "BSF_Darwin_A",
      "kind": "mainstem",
      "colorGroup": "Blacksmith Fork River"
    },
    {
      "id": "seventeen_s",
      "name": "Blacksmith Fork River: 1700 South Footbridge",
      "stationId": "BSF_1700S_A",
      "kind": "mainstem",
      "colorGroup": "Blacksmith Fork River"
    },
    {
      "id": "to_lower_logan",
      "name": "Lower Logan River",
      "kind": "link",
      "linkTo": "lower-logan",
      "label": "Confluence with Logan River →"
    }
  ]
}
```
